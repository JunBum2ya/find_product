...
void __release_packet(IMPacketPtr ppkt)
{
...
int im_action_collectionNumData_send_req_with_trxid(char* dataTypeCd, double snsnVal, long trxid)
{
	int rc = 0;
	IMPacket pkt;
	...
	im_if411_build_numdata_body(pbody, im_base_get_extrSysId(), im_base_get_deviceId(), dataTypeCd, snsnVal);	
...
int im_action_collectionStrData_send_req_with_trxid(char* dataTypeCd, char* snsnVal, long trxid)
{
	int rc = 0;
	IMPacket pkt;
	IMPacketPtr ppkt = &pkt;
	...
	im_if411_build_strdata_body(pbody, im_base_get_extrSysId(), im_base_get_deviceId(), dataTypeCd, snsnVal);	
	rc = im_action_send_packet(ppkt);
    if (rc < 0 ) {
		ERROR_LOG("fail im_action_send_packet(), rc=[%d]", rc);
		return -1;
